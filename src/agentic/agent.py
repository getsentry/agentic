...
from .models import Message, ToolCall, Usage, AgentError
from .tools import FunctionTool
from .utils import parse_json_with_keys
...
class LlmAgent:
    ...
    def get_completion(self, run_config: RunConfig):
        return self.client.generate_text(
            ...
            system_prompt=(
                run_config.system_prompt if run_config.system_prompt else None
            ),
    ...
    def should_continue(self, run_config: RunConfig) -> bool:
        ...
        if last_message and last_message.role in ["assistant", "model"]:
            if last_message.content:
                ...
                if (
                    run_config.stop_message
                    and run_config.stop_message in last_message.content
                ):
                    return False
    ...
    def run(self, run_config: RunConfig):
        if run_config.run_name:
            langfuse_context.update_current_observation(
                name=run_config.run_name + " - Agent Run"
            )
        elif self.name:
